...
def train(args, debug = False):
    ...
    if not debug:
        wandb.init(project="multi_label_celeba_topkset2", group=args.label, config=args, reinit=True)
    ...
    celeba_train_data = CELEBA_PARTIAL(root = args.root, split = "train",
    ...
    celeba_val_data = CELEBA_PARTIAL(root = args.root, split = "val",
                            is_transform=True, transform=transforms.Compose([
                    transforms.ToTensor(),
                    transforms.Resize(size=(IMG_SIZE,IMG_SIZE)),
                    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_storage', type=str, default="/data/mariammaa/celeba/checkpoints_topk_set2/", help='model folder to save the model')
