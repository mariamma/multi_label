...
def train(args):
    ...
    nih_pathFileVal = "/scratch/mariamma/xraysetu/dataset/nih_val_short2.csv"
    ...
    for batch_no, (img_name, images, labels) in enumerate(tqdm(dataLoaderVal)):
        ...
        img_name_pert = img_name[0].split("/")[-1]
        perturbed_image = np.load(os.path.join(args.image_save_dir, img_name_pert.replace(".png", "") + ".npy"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--label', type=str, default='nih', help='wandb group')
    ...
    parser.add_argument('--model_folder', type=str, default="/data/mariammaa/nih_multi_label/checkpoint_nih_topk/", help='model folder to save the model')
    ...
    parser.add_argument('--sal_method', type=str, default="saliency_map", help=' tasks',
